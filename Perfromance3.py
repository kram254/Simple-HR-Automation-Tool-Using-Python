...
class PerformanceEvaluationUI(QMainWindow):
    ...
    def create_table(self):
        try:
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS industries (name TEXT, kpis TEXT)")
    ...
    def evaluate_performance(self):
        ...
        try:
            cursor = self.db_connection.cursor()
            cursor.execute("INSERT INTO industries VALUES (?, ?) ON CONFLICT(name) DO UPDATE SET kpis=?",
                           (selected_industry, kpis, kpis))
            self.db_connection.commit()
        except sqlite3.Error as e:
            print("Database error:", str(e))
